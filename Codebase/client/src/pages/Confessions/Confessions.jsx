...
const SAMPLE_CONFESSIONS = [
  {
    id: 1,
    content: "I've been pretending to understand quantum physics in class for the past 3 weeks. The professor thinks I'm brilliant, but I'm just nodding and saying 'fascinating' at random intervals. 😅",
    tag: "Academic Stress",
  ...
  {
    id: 2,
    content: "My roommate and I have been having a silent war over the thermostat. I like it cold, they like it warm. We've been secretly adjusting it when the other isn't looking. It's been 2 months of this passive-aggressive battle. 🥶",
    tag: "Hall Life",
    timestamp: new Date(Date.now() - 6 * 60 * 60 * 1000), // 6 hours ago
    reactions: { like: 89, funny: 156, relatable: 234, angry: 5, insightful: 23 },
    status: "approved"
  },
  {
    id: 3,
    content: "I accidentally sent a love confession meant for my crush to my professor instead. The subject line was 'I can't stop thinking about you' and I only realized after hitting send. I'm considering transferring to a different university. 😭",
    tag: "Funny",
    timestamp: new Date(Date.now() - 12 * 60 * 60 * 1000), // 12 hours ago
    reactions: { like: 234, funny: 567, relatable: 89, angry: 12, insightful: 45 },
    status: "approved"
  },
  {
    id: 4,
    content: "I found a stray cat near the library and secretly feed it every day. I've named it Professor Whiskers and it's become my unofficial therapy session. Sometimes I tell it about my problems and it just purrs. Best listener ever. 🐱",
    tag: "Wholesome",
    timestamp: new Date(Date.now() - 24 * 60 * 60 * 1000), // 1 day ago
    reactions: { like: 456, funny: 23, relatable: 123, angry: 1, insightful: 78 },
    poll: {
  ...
  {
    id: 5,
    content: "I've been using the same pen for 3 years. It's not even a good pen, but I'm emotionally attached to it. I've lost it twice and had mini panic attacks until I found it. My friends think I'm weird, but this pen has been through everything with me. ✏️",
    tag: "Wholesome",
    timestamp: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000), // 2 days ago
    reactions: { like: 189, funny: 67, relatable: 234, angry: 3, insightful: 45 },
    status: "approved"
  }
];
...
export default function Confessions() {
  ...
  const filteredAndSortedConfessions = useMemo(() => {
    let filtered = confessions.filter(confession => 
    ...
    setConfessions(prev => 
      prev.map(confession => 
        confession.id === confessionId 
          ? { ...confession, reactions: { ...confession.reactions, [reactionType]: confession.reactions[reactionType] + 1 } }
          : confession
      )
    );
  };
  ...
  const getRandomConfession = () => {
    ...
    return approvedConfessions[Math.floor(Math.random() * approvedConfessions.length)];
  };
  ...
  const analyticsData = useMemo(() => {
    ...
    const mostReacted = confessions.reduce((max, confession) => {
      const total = Object.values(confession.reactions).reduce((sum, val) => sum + val, 0);
      return total > max.total ? { confession, total } : max;
    }, { confession: null, total: 0 });

    const mostVotedPoll = confessions
      .filter(c => c.poll)
      .reduce((max, confession) => {
        return confession.poll.totalVotes > max.poll.totalVotes ? confession : max;
      }, { poll: { totalVotes: 0 } });
      ...
      <div className="confessions-main animate-fade-in-up">
        ...
        <div className="results-info">
          <span className="results-count">
            {filteredAndSortedConfessions.length} confessions found
          </span>
        </div>
